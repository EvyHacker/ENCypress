...
describe('test partial refund for IATS UK gateway for Single and Recurring transactions', () => {
  ...
  const donationRecur = ('en_iats_uk_partial_recur_donation_' + tomorrowsDate + '@engagingnetworks.online')
  ...
  it('can submit single donation', () => {
    ...
    cy.get('@thankYouPage').contains('£1.00')
...
describe('test partial refund for single and recurring transactions', () => {
  ...
  it('searches for the singl transaction and completes partial refund', () => {
    ...
    EN.refund().click()
    cy.get('.gadget__receipt > p').invoke('text').should('include', 'Amount Charged: 1 GBP')
    EN.refundAmount().type('1.00')
    ...
    cy.get('@refund').should('include', '-1 GBP')
  ...
  it('searches for the recurring transaction and completes partial refund', () => {
    ...
    cy.get('.gadget__transactionHistory__transactionDetail').invoke('text').should('include', '-1 GBP')
    EN.logOut()
